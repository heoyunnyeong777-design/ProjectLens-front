...
function Content({ active, project, analysis }) {
  if (!analysis) return <div className={styles.empty}>Î∂ÑÏÑù Í≤∞Í≥ºÍ∞Ä ?ÜÏñ¥??</div>

  if (active === 'overview') return <OverviewContent project={project} analysis={analysis} />
  if (active === 'structure') return <SingleContent title="Architecture" emoji="?èóÔ∏? content={analysis.structure} />
  if (active === 'features')  return <SingleContent title="Key Components" emoji="?ôÔ∏è" content={analysis.features} />
  if (active === 'improvements') return <SingleContent title="Improvements" emoji="?îß" content={analysis.improvements} />
    ...
    <div>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDesc}>
          {project?.name} ?ÑÎ°ú?ùÌä∏???ÑÌÇ§?çÏ≤ò, ?µÏã¨ Í∏∞Îä•, Í∞úÏÑ†?êÏùÑ Ï¢ÖÌï© Î∂ÑÏÑù??Í≤∞Í≥º?àÏöî.
        </p>
        <div className={styles.metaRow}>
          <div className={styles.metaBadge}>
            <span className={styles.metaDot} />Î∞©Í∏à Î∂ÑÏÑù??
          </div>
          <div className={styles.metaBadge}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
              <path d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z" stroke="#8b949e" strokeWidth="2"/>
            </svg>
            ?åÏùº {project?.file_count}Í∞?
          </div>
        </div>
      </div>

      <AnalysisCard
        emoji="?èóÔ∏? title="Architecture"
        content={analysis.structure}
        color="blue"
      />
      <AnalysisCard
        emoji="?ôÔ∏è" title="Key Features"
        content={analysis.features}
        color="purple"
      />
      <AnalysisCard
        emoji="?îß" title="Improvements"
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDesc}>?ÑÏ≤¥ Î∂ÑÏÑù Í≤∞Í≥ºÎ•?Ï¢ÖÌï©??ÏµúÏ¢Ö Î≥¥Í≥†?úÏòà??</p>
      </div>
...
/* ?Ä?Ä Î∂ÑÏÑù Ïπ¥Îìú ?Ä?Ä */
function AnalysisCard({ emoji, title, content, color }) {
  const paragraphs = (content || 'Î∂ÑÏÑù Ï§?..').split('\n').filter(l => l.trim())
