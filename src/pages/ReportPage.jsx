    ...
    const load = async () => {
      try {
        const [projects, analysisRes] = await Promise.all([
          ...
          axios.get(`http://localhost:8000/api/projects/${projectId}/analysis`)
        ])
...
function Content({ active, project, analysis }) {
  ...
  if (active === 'features')  return <SingleContent title="Key Components" emoji="⚙️" content={analysis.features} />
    ...
    <div>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDesc}>
          {project?.name} 프로젝트의 아키텍처, 핵심 기능, 개선점을 종합 분석한 결과예요.
        </p>
        <div className={styles.metaRow}>
          <div className={styles.metaBadge}>
            <span className={styles.metaDot} />방금 분석됨
          </div>
          <div className={styles.metaBadge}>
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.pageDesc}>전체 분석 결과를 종합한 최종 보고서예요.</p>
      </div>
